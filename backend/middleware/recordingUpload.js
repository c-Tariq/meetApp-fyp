...
const fileFilter = (req, file, cb) => {
    ...
    if (file.mimetype === 'video/webm' || file.mimetype === 'video/mp4' || file.mimetype === 'audio/webm') {
        cb(null, true); // Accept file
    } else {
        console.warn(`Rejected file type: ${file.mimetype}`);
        cb(new Error('Invalid file type. Only webm or mp4 video/audio allowed.'), false); // Reject file
    }
};
