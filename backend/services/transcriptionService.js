...
const transcriptionService = {
   ...
   */
  async getTranscriptFromUrl(recordingUrl) {
    // Renamed for clarity
    console.log(
      `[DEPRECATED] Attempting transcription from URL: ${recordingUrl}`
    );
    ...
    try {
      ...
      if (response.data && typeof response.data.text === "string") {
        ...
        throw new Error(
          "Transcription failed: Invalid response format from API."
        );
      }
    } catch (error) {
      const errorMessage =
  ...
  async getTranscriptFromFileData(audioBuffer, originalFileName) {
    console.log(
      `Attempting direct file transcription for: ${originalFileName}`
    );
    if (!ELEVENLABS_API_KEY) {
      console.error("ELEVENLABS_API_KEY environment variable not set.");
      throw new Error("Transcription service API key not configured.");
