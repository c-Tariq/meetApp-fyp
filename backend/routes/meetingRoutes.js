...
const createMeetingValidation = [
    ...
    body('scheduled_time', 'Scheduled time must be a valid ISO 8601 date string').isISO8601().toDate()
];
const updateStatusValidation = [
    body('status', 'Status is required and must be valid').isIn(['Upcoming', 'In Progress', 'Concluded'])
];
const transcriptBodyValidation = [
    body('message', 'Transcript text (message) is required in the body').notEmpty().isString()
];
...
const updateMeetingDetailsValidation = [
    ...
    body('scheduled_time').optional().isISO8601().toDate().withMessage('Scheduled time must be a valid ISO 8601 date string if provided')
...
router.post('/:meetingId/process-transcript', 
    ensureAuthenticated, 
    meetingIdValidation,
...
router.post('/:meetingId/recording', 
    ensureAuthenticated, 
    meetingIdValidation, 
...
); // POST /spaces/:spaceId/meetings/:meetingId/recording
